...
class RedBlackTreeMap(TreeMap):
    ...
    class _Node(TreeMap._Node):
        """Node class for red-black tree maintains bit that denoted color."""
    ...
    def _resolve_red(self, p):
        if self.is_root(p):
            ...
            if self._is_red(parent):    # double red problem
                ...
                if not self._is_red(uncle): # Case 1: misshaped 4-node
                    middle = self._restructure(p)   # do trinode restructuring
                    self._set_black(middle)         # fix colors
                    self._self_red(self.left(middle))
                    self._self_red(self.right(middle))
    ...
    def _rebalance_delete(self, p):
        if len(self) == 1:
            ...
            if n == 1:                      # deficit exits unless child is a red leaf
        ...
        if not self._is_red(y): # y is black; will apply Case 1 or 2
            ...
            if x is not None:   # Case 1: y is black and has red child x; do "transfer"
                ...
                if self._is_red(z):
                    ...
                    self._fix_deficit(self.parent(z), self.sibling(z)) # recursion upward
